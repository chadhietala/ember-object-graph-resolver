...
function getModelFor (node) {
  if ( node.type === 'CallExpression' && _keyExists(node, 'callee', 'property') && node.callee.property.name === 'modelFor' ) {
    return node.arguments[0].value;
  }
}
...
function getView (node) {
  ...
  if (node.type === 'mustache' && sexpr.id.original === 'view') {
  ...
  traverse(nodes, {
    pre: function(node) {
      var controllerFor = getControllerFor(node),
          ...
          modelFor = getModelFor(node),
          controllers = this.controllers,
          _fullNames = this._fullNames;

      if (controllerFor) {
        controllers[controllerFor] = 'controller:' + controllerFor;
        _fullNames.push(controllers[controllerFor]);
      }

      if (modelFor) {
        controllers[modelFor] = 'controller:' + modelFor;
        _fullNames.push(controllers[modelFor]);
      }
